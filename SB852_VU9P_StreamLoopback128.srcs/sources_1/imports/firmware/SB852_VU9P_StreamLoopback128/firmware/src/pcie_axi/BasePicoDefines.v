...
`ifdef ENABLE_QSPI_FLASH 
    `define FLASH_CTL_STREAM_ID     115     // use stream ID 115
    `define STREAM115_IN_WIDTH      128
    `define STREAM115_OUT_WIDTH     128
`endif
`ifdef ENABLE_HMC
...
//--------------------------Memory---------------------------------------------------------------------------
`ifdef PICO_DDR3
    `ifndef PICO_DDR3_OR_DDR4
        `define PICO_DDR3_OR_DDR4
    `endif
...
// Firmware supports a 256 bit maximum stream width
`define STREAM_MAX_256BIT

`ifdef PCIE_GEN3X16
   // PCIE_GEN3X16 requires PCIE_GEN3X8 support logic also
   `define PCIE_GEN3X8
   `ifdef STREAM_MAX_256BIT
      `define PCIE_GEN3X16_STREAM_MAX_256BIT
   `endif
`endif

`ifdef PCIE_GEN3X16
   `define PCIE_GEN3X16_USER_CLOCK
`endif
...
`elsif PICO_MODEL_M510
    ...
    `ifndef PICO_MODEL_M510_OR_M520
        `define PICO_MODEL_M510_OR_M520
    `endif
`elsif PICO_MODEL_EX800
    `define PICO_MODEL_NUM  32'h800
`elsif PICO_MODEL_M520
    `define PICO_MODEL_NUM  32'h520
    `ifndef PICO_MODEL_M510_OR_M520
        `define PICO_MODEL_M510_OR_M520
    `endif
`elsif PICO_MODEL_AC511
    `define PICO_MODEL_NUM  32'h511
`elsif PICO_MODEL_EAGLE5
    `define PICO_MODEL_NUM  32'hEA5
`elsif PICO_MODEL_SB851
    `define PICO_MODEL_NUM  32'h851
`elsif PICO_MODEL_SB852
    `define PICO_MODEL_NUM  32'h852
`else
    `ERROR_NO_PICO_MODEL
`endif
